...
lib.onBothPlayerAnswers = function (firstPlayerAnswers,secondPlayerAnswers) {
	var firstPlayerPoints = calculatePoints(firstPlayerAnswers,10);
	var secondPlayerPoints = calculatePoints(secondPlayerAnswers,10);
	var gameStatus = lib.authenticateToFirebase("quizup.firebaseio.com/doNotChange/game/gameStatus","lLwXjNDm5algYXbUEEWekyVr30cgH9nQVW3yiDAw");
...
lib.putResultAfterCompletion = function (gameUrl,token) {
	...
	var addPlayerOneAnswerListener = function (callback) {
		player1Ref.on('value',function (snapshot) {
			...
			if(secondPlayerAnswers)
				callback(firstPlayerAnswers,secondPlayerAnswers);
		});	
	};

	var addPlayerTwoAnswerListener = function (callback) {
	...
	addPlayerOneAnswerListener(lib.onBothPlayerAnswers);
	addPlayerTwoAnswerListener(lib.onBothPlayerAnswers);
}
